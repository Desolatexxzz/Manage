...
export default {
  ...
  methods: {
    ...
    doAddDep() {
      if (this.dep.name) {
        this.postRequest('/system/basic/department/addDepartment', this.dep).then(resp => {
          if (resp) {
    ...
    deleteDep(node, data) {
      if (data.isParent) {
        ...
        this.$confirm('此操作将删除该[' + data.name + ']部门, 是否继续?', '提示', {
          ...
          this.deleteRequest('/system/basic/department/deleteDepartment/' + data.id).then(resp => {
            if (resp) {
