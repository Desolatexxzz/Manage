  ...
  methods:{
    deleteMany() {
      this.$confirm('此操作将永久删除[' + this.multipleSelection.length + ']条职位, 是否继续?', '提示', {
        ...
        this.deleteRequest('/system/basic/pos/deletePositions' + ids).then(resp => {
          if (resp) {
            this.initPositions();
          }
        })
    ...
    doUpdate() {
      this.putRequest('/system/basic/pos/updatePosition', this.updatePos).then(resp => {
        if (resp) {
          this.initPositions();
    ...
    addPosition() {
      if (this.pos.name) {
        this.postRequest('/system/basic/pos/addPosition', this.pos).then(resp => {
          if (resp) {
            this.initPositions();
            this.pos.name = '';
          }
    ...
    handleDelete(index, data) {
      this.$confirm('此操作将永久删除[' + data.name + ']该职位, 是否继续?', '提示', {
        ...
        this.deleteRequest('/system/basic/pos/deletePosition/' + data.id).then(resp => {
          if (resp) {
            this.initPositions();
          }
        })
