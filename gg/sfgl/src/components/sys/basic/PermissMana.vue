  ...
  methods: {
    doDeleteRole(role){
      this.$confirm('此操作将永久删除[' + role.nameZh + '], 是否继续?', '提示', {
        ...
        this.deleteRequest('/system/basic/permiss/role/deleteRole/' + role.id).then(resp => {
          if (resp) {
            this.initRoles();
          }
        })
    ...
    doAddRole() {
      if (this.role.nameZh && this.role.name) {
        this.postRequest('/system/basic/permiss/role/addRole',this.role).then(resp => {
          if (resp) {
            this.initRoles();
            this.role.name = '';
            this.role.nameZh = '';
    ...
    doUpdate(rid, index) {
      ...
      let url = '/system/basic/permiss/updateRoleMenu/?rid=' + rid;
    ...
    initSelectedMenus(rid) {
      this.getRequest('/system/basic/permiss/menu/mid/' + rid).then(resp => {
        if (resp) {
