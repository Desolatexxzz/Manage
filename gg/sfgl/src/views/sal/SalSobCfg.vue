  ...
  <div>
    <el-table :data="emps" stripe>
      ...
      <el-table-column
          ...
          fixed
      ></el-table-column>
      <el-table-column prop="workID" width="120" label="工号"></el-table-column>
      <el-table-column
          prop="email"
          width="200"
          label="邮箱地址"
  ...
  methods: {
    hidePop(data) {
      if (this.currentSalary && this.currentSalary != data.salary.id) {
        this.putRequest(
            "/salary/sobcfg/updateEmployeeSalary/?eid=" + data.id + "&sid=" + this.currentSalary
        ).then((resp) => {
          if (resp) {
            this.initEmps();
          }
    ...
    initSalaries() {
      this.getRequest("/salary/sobcfg/getAllSalary").then((resp) => {
        if (resp) {
          this.salaries = resp;
        }
      });
